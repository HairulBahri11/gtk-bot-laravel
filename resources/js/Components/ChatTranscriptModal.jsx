...
/**
 * Modal transkrip chat WhatsApp - dipanggil dari halaman Antrean (gabungan
 * Antrean + Monitoring), tidak lagi navigasi ke halaman terpisah. Fetch
 * data on-demand lewat monitoring.show (JSON) begitu chatSessionId diisi.
 */
export default function ChatTranscriptModal({ chatSessionId, onClose }) {
    ...
    useEffect(() => {
        ...
        axios
            .get(route('monitoring.show', chatSessionId))
            .then((res) => setSession(res.data.session))
            .catch(() => setError('Gagal memuat transkrip percakapan.'))
            .finally(() => setLoading(false));
    }, [chatSessionId]);

    const dayGroups = useMemo(
        () => (session ? groupMessages(session.messages) : []),
        [session],
    );
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative flex h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-xl bg-white shadow-xl dark:bg-gray-900">
                <div className="flex shrink-0 items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-800">
                    <div>
                        <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                            {session?.nomor_wa ?? 'Transkrip Percakapan'}
                        </h3>
                        {session && (
                ...
                {session && !loading && !error && (
                    <div className="grid flex-1 grid-cols-1 overflow-hidden lg:grid-cols-3">
                        <div className="space-y-4 overflow-y-auto border-b border-gray-200 p-4 dark:border-gray-800 lg:col-span-1 lg:border-b-0 lg:border-r">
                            {session.patient && (
                                <div>
                                    ...
                                    <dl className="mt-2 space-y-1 text-sm text-gray-900 dark:text-gray-100">
                                        <div>
                                            <dt className="inline text-gray-500">No. RM: </dt>
                                            <dd className="inline">{session.patient.no_rm}</dd>
                                        </div>
                                        <div>
                                            <dt className="inline text-gray-500">Nama: </dt>
                                            <dd className="inline">{session.patient.nama}</dd>
                                        </div>
                                        <div>
                                            <dt className="inline text-gray-500">Tanggal Lahir: </dt>
                                            <dd className="inline">{session.patient.tanggal_lahir}</dd>
                                        </div>
                                        <div>
                                            <dt className="inline text-gray-500">Nama Ibu: </dt>
                                            <dd className="inline">{session.patient.nama_ibu_kandung}</dd>
                                        </div>
                                        <div>
                                            <dt className="inline text-gray-500">Nomor WA: </dt>
                                            <dd className="inline">
                                                {session.patient.no_hp ?? (
                                                    <span className="text-gray-400">Nomor tidak terdeteksi</span>
                                                )}
                                            </dd>
                                        </div>
                                    </dl>
                            ...
                            <div>
                                ...
                                {session.bookings.length === 0 && (
                                    <p className="mt-2 text-sm text-gray-500">Belum ada booking.</p>
                                )}
                                            ...
                                            >
                                                {m.isFirstOfBlock && (
                                                    <span className="mb-1 px-1 text-xs font-medium text-gray-400 dark:text-gray-500">
                                                        {m.direction === 'out' ? 'AI Pre-Layanan' : 'Orang Tua Pasien'}
                                                    </span>
                                                )}
                                                <div
